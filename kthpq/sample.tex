...
\begin{frame}{What are we talking about}
	...
	\only<1>{
		\begin{textblock*}{20pt}(550pt,350pt)
			\small
			\cite{Xue2022}
		\end{textblock*}}
...
\begin{frame}{Control Concept}
	...
	\end{itemize}
\todo{fix the layouting of this page }
	...
	\begin{columns}
		...
		\begin{column}{0.45\textwidth}
			...
			\begin{itemize}
				...
				\item Biologically plausible pre$\times$post locally
...
\begin{frame}{Examples}
	\begin{columns}
		\begin{column}{0.45\textwidth}
			%\includegraphics[width= 0.9\textwidth]{Learned_results.png}
			\todo{Insert Picture}
		\end{column}
		\begin{column}{0.45\textwidth}
			\todo{Insert Picture}
			%\includegraphics[width=0.9\textwidth]{Over time.png}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}
	...
	\only<4>{\begin{textblock*}{50pt}(450pt,30pt)
		...
		\only<3,4>{Orthonormality restriction on Input Matrix $B \in \mathbb{B} := $$\left\{M\ |\ M^TM = \mathbf{I}\right\}$\\}
...
\begin{frame}{Examples}
	\begin{columns}
		\begin{column}{0.45\textwidth}
			\todo{1 Example working with single Network working}
		\end{column}
		\begin{column}{0.45\textwidth}
			\todo{1 Example of 2 networks working}
		\end{column}
	\end{columns}
\end{frame}
