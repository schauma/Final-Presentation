...
\begin{frame}{Problems}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item No compensation of Training errors
		\end{itemize}
		\item No biolically plausible Learning rule for control network available
	\end{itemize}
\end{frame}
...
\begin{frame}
	\only<1>{\frametitle{Dual Network with Feedback}}
	\only<2>{\frametitle{Single Network}}
	\only<3>{\frametitle{Single Network with Feedback}}
	...
	\only<3>{\begin{textblock*}{50pt}(450pt,30pt)
		...
		\only<1>{No Learning rule for control network available\\}
		\only<2,3>{\st{No Learning rule for control network available}\\}

		\only<2>{Open Loop Control:\\ \begin{itemize}
				\item Incapable of noise detection or correction
				\item No Compensation of Training error
		\end{itemize}}
		\only<1,3>{\st{Open loop Control:}\\\begin{itemize}
				\item \st{Incapable of noise detection or correction}
				\item \st{No Compensation of Training error}
		\end{itemize}}

		\only<2>{Highly dependent on governing dynamics from $c_{\text{contr}} = \dot{x}_{\text{ref}}-Ax_{\text{ref}}$\\}
		\only<1,3>{\st{Highly dependent on governing dynamics from $c_{\text{contr}} = \dot{x}_{\text{ref}}-Ax_{\text{ref}}$}\\}
		\only<2,3>{Orthonormality restriction on Input Matrix $B \in \mathbb{B} := $$\left\{M\ |\ MM^T = \mathbf{I}\right\}$\\}
